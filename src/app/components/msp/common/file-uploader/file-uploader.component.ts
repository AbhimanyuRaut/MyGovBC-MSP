...
export class FileUploaderComponent
    ...
    observableFromFiles(fileList: FileList, scaleFactors: MspImageScaleFactors) {
        ...
        // Init
        let self = this;
        var  pageNumber = Math.max(...self.images.map(function(o){return o.attachmentOrder;}),0) +1  ;  // get current max page number
        ...
        let fileObservable = Observable.create((observer: Observer<MspImage>) => {
            ...
            for (var fileIndex = 0; fileIndex < fileList.length; fileIndex++) {
                ...
                if (file.type == "application/pdf") {
                    ...
                    this.readImage(file, pageNumber , (image: HTMLImageElement ,imageFile: File ,pageNumber: number )  => {
                            image.id = imageFile.name; //.name deprecated, changed image.name to image.id
                            this.resizeImage(image, self, scaleFactors, observer , pageNumber );
                        },
...
// While it's still in an image, get it's height and width
        let mspImage: MspImage = new MspImage();
        let reader: FileReader = new FileReader();
            console.log("image.name:"+image.id+"pageNumber:"+pageNumber); //.name deprecated, changed image.name to image.id
        // Copy file properties
